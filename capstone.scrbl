...
modeled with a finite state machine. A finite state machine is simple to
implement, but regular expressions cannot parse most programming
languages. They're usually used for simple pattern matching on strings,
and are used in the lexing stage of many parsers.
...
without requiring the more sophisticated parsing algorithms required for
the languages higher in the Chomsky hierarchy.

Context-sensitive languages can be parsed with a restricted Turing machine
and a recursively-enumerable language can be parsed with a Turing machine.
